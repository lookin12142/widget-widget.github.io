...
(function(){
  ...
  function SmartclicWidget(opts) {
    ...
    const header = createEl('div', {class:'sc-header'}, [
      ...
      createEl('div', {class:'sc-actions'}, [
        ...
        createEl('button',{class:'sc-iconbtn', title:'Minimizar'}, [createEl('span',{text:'−'})]),
    ...
    // Event handlers
    const [clearBtn, minimizeBtn, closeBtn] = header.querySelectorAll('.sc-iconbtn');
    ...
    // Public API
    return {
      open: () => panel.classList.add('sc-open'),
      close: closePanel,
      toggle: togglePanel,
      clear: clearHistory,
      minimize: () => { if (!isMinimized) toggleMinimize(); },
      restore: () => { if (isMinimized) toggleMinimize(); },
      element: root,
      config: cfg,
  ...
  window.SmartclicChat = {
    ...
    // Utility to load widget from external source
    loadFromUrl: (url, opts = {}) => {
      return new Promise((resolve, reject) => {
        const script = document.createElement('script');
        script.src = url;
        script.onload = () => {
          resolve(SmartclicChat.init(opts));
        };
    ...
    create: (id, opts) => {
      if (SmartclicChat.instances.has(id)) {
        console.warn(`SmartClic instance '${id}' already exists`);
        return SmartclicChat.instances.get(id);
      }
      
      const instance = new SmartclicWidget(opts);
      SmartclicChat.instances.set(id, instance);
      return instance;
    },
    ...
    destroy: (id) => {
      const instance = SmartclicChat.instances.get(id);
      if (instance && instance.element) {
        instance.element.remove();
        SmartclicChat.instances.delete(id);
        return true;
      }
      return false;
    }
